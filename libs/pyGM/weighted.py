...
class WeightedModel(GraphModel):
  ...
  def _bucket_of(self,f):
    ...
    return self._elim_order[min( self.priority[f.v.labels] )]   # TODO: unittests / check correctness
  ...
  def update(self, stopIter=100, stopTime=inf, stopTol=0.0, stepInner=5, verbose=False):
    ...
    for t in xrange(1,stopIter+1):               # for each iteration:
        ...
        for Xi in self.elim_order:                # for each variable,  TODO: use elim order
            ...
            #print weight_i thetas,weights,Xi,steps,threshold=1e-4,direction=+1, optTol=1e-8,progTol=1e-8
