...
def run(name="car", pseudo_tree= [-1, 5, 5, 0, 3, 4], ibound=2, tl=3600, it=1, optimize_weight=0, optimize_cost=0):
    ...
    wmb_heur = WeightedMBE(verbose_level, jgd, ordering, weights, is_log, WEPS, log_file_name, ibound, mini_buckets, gm.variables)
    wmb_heur.print_log("WeightedMBE")
    ...
    write_mini_bucket_heuristic_from_info(os.path.join(TEST_PATH, name), heur_info)
