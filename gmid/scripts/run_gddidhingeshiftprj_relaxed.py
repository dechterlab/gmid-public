...
def run(name="pomdp1-4_2_2_2_3", ibound=1, time_limit=100, iter_limit=5, mbe_init=True):
    ...
    #### GDD for id
    gdd_str ='_GddIdHingeShiftProj_i=' + str(ibound) + '_init=' + str(mbe_init)
    log_file_name = os.path.join(LOG_PATH_RELAXED, name+gdd_str)
    cost_options = {
        'gd_steps': 10, 'tol': TOL,
        'ls_steps' : 30, 'armijo_thr' : 1e-4, 'armijo_step_back' : 0.5, 'ls_tol': TOL,
        'max_bcd_scope_size' : 5
    }
    ...
    util_options = {
        'gd_steps': 50, 'tol': TOL,
        'ls_steps': 30, 'armijo_thr': 1e-4, 'armijo_step_back': 0.5, 'ls_tol': TOL,
        'trigger_util_update' : 1e-2, 'util_update_period' : 1, 'no_util_update_iter' : 1
    }
