...
class WeightedMBE3(MessagePassing):
    def __init__(self, verbose_level, message_graph, elim_order, weights, is_log, epsilon, log_file_name, ibound,
        ...
        super(WeightedMBE3, self).__init__(verbose_level, message_graph, elim_order, weights, is_log, log_file_name)
    ...
    def init_propagate(self):
        ...
        # init weights and pseudo beliefs, fully decomposed bounds
        self._reset_uniform_weights()
        self._reset_node_bounds()
        self._reset_pseudo_beliefs()

        # perform 1 gdd weight iteration
        print("initialization of weights by weight update")
        for ind, nodes in enumerate(self.weight_schedule):
            v_w = self.elim_order[ind]
            var_obj = self.gm_variables[v_w]
            if v_w not in self.sum_vars or len(nodes) <= 1:
                continue
            self._optimize_weights(var_obj, nodes)
    ...
    ####################################################################################################################
    def _optimize_weights(self, var_obj, node_to_update):
        if debug:
            print("start updating weight at var {} on {}:".format(var_obj, node_to_update))
        ...
        for s in range(gd_steps):
            gradient = self._eval_weight_gradients(var_obj, node_to_update)
            ...
            if debug:
                ...
                print("\t\t\t\t\tf0={}\t\tf1={}\t\tdec:{}".format(obj_0, obj_1, obj_0 >= obj_1))
            obj_0 = obj_1
            new_weights = self._line_search_weights(var_obj, node_to_update, obj_0, gradient, step, L0, L2)
            ls_updated = False if new_weights is None else True
            if ls_updated:
                self._update_weights(var_obj, new_weights, node_to_update)
