...
class GddIdHingeShiftProjected(MessagePassing):
    def __init__(self, verbose_level, message_graph, elim_order, weights, is_log, epsilon, log_file_name, ibound):
        super(GddIdHingeShiftProjected, self).__init__(verbose_level, message_graph, elim_order, weights, is_log, log_file_name)
    ...
    def bounds(self, changed_nodes=[]):
        ...
        for node in self.message_graph_nodes:
            if node in changed_nodes:   # recompute bounds for relevant nodes
                ...
                # if debug:     # below can take np.inf
    ...
    def propagate(self, time_limit, iter_limit, cost_options, weight_options, util_options):
        ...
        for iter in range(iter_limit):
            ...
            if self.util_const_updated_count > 10 and (self.total_diff < TOL):
                self.print_log("gdd terminating due to diff:{}".format(self.total_diff))
                break
            if self.util_const_updated_count > 10 and (time.time() - self.time_0 > self.time_limit):
                self.print_log("gdd terminating due to time_limit:{}".format(self.time_limit))
                break
        else:
    ...
    def _propagate_one_pass(self, it, cost_options, weight_options, util_options):
        ...
        for edge in self.cost_schedule:
            self._update_cost_per_edge_per_var(edge, cost_options)
        self.new_eu = self.util_shifted_bounds(self.bounds(self.mg.message_graph.nodes()))
        if self.best_expected_utility > self.new_eu:
            self.best_expected_utility = self.new_eu
        if it >= 0:
        ...
        ### util const update
        # if it < 0 or (self.total_diff/self.best_expected_utility < util_options['trigger_util_update']):
        # if it > 0 and (it % 5 == 0):
        # if it > util_options['no_util_update_iter'] and (it % util_options['util_update_period'] ==0):
        print("trigger:{}".format(self.total_diff/one_pass_init))
        if it > util_options['no_util_update_iter'] and self.total_diff/one_pass_init < util_options['trigger_util_update']:
    ...
    ####################################################################################################################
    def _update_cost_per_edge_per_var(self, edge, options):
        if self.terse:
        ...
        sc_all = self.mg.message_graph.edge[node_from][node_to]['sc']
        # if len(sc_all) > self.ibound // 2:
        #     sc = random.sample(sc_all, min(self.ibound // 2 + 1, options['max_bcd_scope_size']))
        if len(sc_all) > options['max_bcd_scope_size']:
            sc = random.sample(sc_all, options['max_bcd_scope_size'])
        else:
            sc = sc_all
        for s in range(gd_steps):
            if self.terse:
            ...
            #### eval gradient
            prob_gradient = self._eval_prob_gradients_per_var(sc, node_from, node_to)
            util_gradient = self._eval_util_gradients_per_var(sc, node_from, node_to)
    ...
    def _eval_util_gradients_per_var(self, sc, node_from, node_to):
    ...
    def _eval_prob_gradients_per_var(self, sc, node_from, node_to):
        # sc = self.mg.message_graph.edge[node_from][node_to]['sc']
